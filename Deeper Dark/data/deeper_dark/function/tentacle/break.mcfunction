...
tag @e[type=block_display,tag=deeper_dark.selected_all] add deeper_dark.silent_despawn
execute as @e[type=block_display,tag=deeper_dark.selected_all] at @s positioned ^ ^.75 ^ run particle minecraft:block{block_state:sculk} ~ ~ ~ .3 .3 .3 .01 20
