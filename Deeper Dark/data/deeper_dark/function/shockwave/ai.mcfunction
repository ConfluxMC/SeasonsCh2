#display entity
execute as @e[type=block_display,tag=deeper_dark.shockwave_display_main] at @s unless entity @n[tag=deeper_dark.shockwave,type=pig,distance=0..1] unless predicate deeper_dark:is_passenger at @s run function deeper_dark:shockwave/death
execute as @e[type=block_display,tag=deeper_dark.shockwave_display_main] at @s unless entity @n[tag=deeper_dark.shockwave,type=pig,distance=0..1] on vehicle run tp @s ~ ~-10000 ~
