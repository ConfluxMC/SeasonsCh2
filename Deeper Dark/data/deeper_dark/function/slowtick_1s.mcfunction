...
#portal
execute as @e[type=#minecraft:item_frames,predicate=deeper_dark:item_echo_shard_key,tag=!deeper_dark.invalid_portal] at @s positioned ^ ^1 ^-.5 rotated ~90 0 align xyz positioned ~.5 ~.5 ~.5 unless entity @e[distance=0...1,tag=deeper_dark.portal_display] run tag @s remove deeper_dark.active_portal
execute as @e[type=#minecraft:item_frames,predicate=deeper_dark:item_echo_shard_key,tag=!deeper_dark.active_portal] at @s positioned ^ ^1 ^-.5 rotated ~90 0 align xyz positioned ~.5 ~.5 ~.5 if entity @e[distance=0...1,tag=deeper_dark.portal_display] run tag @s add deeper_dark.active_portal
...
execute as @e[type=#minecraft:item_frames,predicate=deeper_dark:item_echo_shard_key,tag=!deeper_dark.invalid_portal] at @s if entity @e[tag=deeper_dark.portal_activation,distance=0..1] unless entity @e[tag=deeper_dark.portal_display,distance=0..1.5] run function deeper_dark:portal/portal
...
execute as @a[predicate=deeper_dark:advancement_undermine] run advancement grant @s only deeper_dark:undermine
