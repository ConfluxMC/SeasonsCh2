...
# tag all targets not at a sculk block for deletion
execute as @e[type=marker,tag=deeper_dark.anticatalyst_target] at @s unless block ~ ~ ~ #deeper_dark:sculk run tag @s add deeper_dark.silent_despawn
...
# rotate towards the nearest valid target (then tag it for deletion)
...
# if var is 1 (1st loop), instead rotate towards nearest sculk entity
...
tp @e[type=marker,tag=deeper_dark.anticatalyst_particle] ~ -1000 ~
#say fire
