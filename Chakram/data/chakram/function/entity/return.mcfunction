...
execute at @s as @s[predicate=chakram:is_dark] if entity @p[distance=..100,gamemode=!spectator,predicate=chakram:idmatch,tag=!chakram.shadowstep_under_effects] if data entity @s item.components."minecraft:enchantments"."chakram:shadowstep" if score @s chakram_bounces matches 1.. run return run function chakram:enchants/shadowstep/run
