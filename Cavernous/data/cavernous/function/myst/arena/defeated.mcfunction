bossbar set high_priest_health players
bossbar set high_priest_health color red
scoreboard players set @n[type=marker,tag=return_portal] return_portal.active 1

loot spawn ~ ~1 ~ loot cavernous:myst/high_priest_loot

playsound minecraft:block.end_portal.spawn block @a ~ ~ ~ 1 2
playsound minecraft:block.sculk_shrieker.shriek hostile @a ~ ~ ~ 1 0.4
tellraw @a[distance=..50] ["",{"text":"<","color":"white"},{"text":"High Priest Veridian","bold":true,"color":"red"},{"text":"> NOOOOOOOOOOOOO","color":"white"}]
particle dust{color:[0.443, 0.055, 0.055],scale:4} ~ ~1.5 ~ 1.7 1.7 1.7 0.2 200 normal @a

advancement grant @a[distance=..50] only cavernous:expansion/myst/priest_defeat

kill @s