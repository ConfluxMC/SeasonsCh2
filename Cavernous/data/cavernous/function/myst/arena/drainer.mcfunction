...
execute if score .global priest.active_global matches 1 positioned ~ ~1 ~ unless entity @e[type=skeleton,tag=priest_2,distance=..2] run return run tellraw @a[distance=..50] {"text":"[DRAINER] ERROR - NO TARGET FOUND","color":"gray"}
